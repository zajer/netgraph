...
void construct_target_graph_for_subiso_test1(struct NatGraph *graph){
    //structure:
    //  0
    // 1 2
    //3
...
void construct_pattern_graph_for_subiso_test1(struct NatGraph *graph){
    //structure:
    //  0
    // 1 2
    // 0 -> 1, 2
...
CTEST(all_subiso_suite,no_colors_not_directed){
    ...
    construct_target_graph_for_subiso_test1(target);
...
void color_target_graph_for_subiso_test3(struct NatGraph *graph){
...
void color_pattern_graph_for_subiso_test3(struct NatGraph *graph){
...
CTEST(all_subiso_suite,colors_not_directed){
    ...
    construct_target_graph_for_subiso_test1(target);
    construct_pattern_graph_for_subiso_test1(pattern);
    color_target_graph_for_subiso_test3(target);
    color_pattern_graph_for_subiso_test3(pattern);
    result = get_all_subisos(*target,*pattern,0);
    ASSERT_EQUAL(expected_number_of_isos,result.number_of_isos);
    free(target->adjacency_data);
...
void construct_target_graph_for_iso_test1(struct NatGraph *graph){
    //structure:
    // 0 1
    //  2
    //  3
...
void construct_pattern_graph_for_iso_test1(struct NatGraph *graph){
    //structure:
    // 2 3
    //  1
    //  0
...
CTEST(all_isos_suite,no_colors_not_directed){
    struct FoundIsomorphisms result;
    struct NatGraph *target = malloc (sizeof( struct NatGraph ));
    struct NatGraph *pattern = malloc (sizeof( struct NatGraph ));
    int expected_number_of_isos = 6;
    construct_target_graph_for_iso_test1(target);
    construct_pattern_graph_for_iso_test1(pattern);
...
CTEST(all_isos_suite,no_colors_directed){
    ...
    construct_target_graph_for_iso_test1(target);
    construct_pattern_graph_for_iso_test1(pattern);
    result = get_all_isos(*target,*pattern,1);
    ASSERT_EQUAL(expected_number_of_isos,result.number_of_isos);
    free(target->adjacency_data);
    free(pattern->adjacency_data);
    for(int i=0;i<expected_number_of_isos;i++)
...
void color_target_graph_for_iso_test3(struct NatGraph *graph){
    int ref_colors[] = {0,1,1,0};
    int ref_colors_length = sizeof(ref_colors)/sizeof(int);
    int *colors = malloc (sizeof(int)*ref_colors_length);
    for(int i=0;i<ref_colors_length;i++)
        colors[i]=ref_colors[i];
...
void color_pattern_graph_for_iso_test3(struct NatGraph *graph){
    int ref_colors[] = {0,1,0,1};
    int ref_colors_length = sizeof(ref_colors)/sizeof(int);
    int *colors = malloc (sizeof(int)*ref_colors_length);
    for(int i=0;i<ref_colors_length;i++)
        colors[i]=ref_colors[i];
...
CTEST(all_iso_suite,colors_not_directed){
    struct FoundIsomorphisms result;
    struct NatGraph *target = malloc (sizeof( struct NatGraph ));
    struct NatGraph *pattern = malloc (sizeof( struct NatGraph ));
    int expected_number_of_isos = 2;
    construct_target_graph_for_iso_test1(target);
    construct_pattern_graph_for_iso_test1(pattern);
    color_target_graph_for_iso_test3(target);
    color_pattern_graph_for_iso_test3(pattern);
    result = get_all_isos(*target,*pattern,0);
    ASSERT_EQUAL(expected_number_of_isos,result.number_of_isos);
    free(target->adjacency_data);
    free(target->vertices_colors);
...
CTEST(all_iso_suite,colors_directed){
    struct FoundIsomorphisms result;
    struct NatGraph *target = malloc (sizeof( struct NatGraph ));
    struct NatGraph *pattern = malloc (sizeof( struct NatGraph ));
    int expected_number_of_isos = 1;
